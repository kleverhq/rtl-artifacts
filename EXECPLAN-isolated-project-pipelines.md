...
- [x] Implement Milestone 12: built the systemc-components image, downloaded SCC, prepared/patched/built SCC examples, collected 29 non-empty VCD/FST/FTR artifacts, and verified old non-systemc artifacts were unchanged.
- [x] Ran implementation review lanes for code/build, architecture/release, and docs after the first successful SCC collection.
- [x] Applied review fixes for stale download/prepare stamps, partial publication state, outdated retrospective text, absolute path validation wording, and missing PyPI/Conan network prerequisites.
- [x] Found and fixed a Make recipe guard bug where separate shell lines ignored `exit 0`; `projects/systemc-components/Makefile` now uses `.ONESHELL` and trimmed executable counts so incremental no-op works.
- [x] Revalidated after review fixes: full collect succeeded, old 33 non-systemc artifacts were preserved, project list/find matched at 29 paths, full list/find matched at 62 paths, all SCC artifacts were non-empty, and a second incremental collect produced zero output with unchanged stamps.
- [x] Ran targeted re-review/control pass. Docs recheck was clean; code recheck found the missing-build-directory `find|wc` guard issue.
- [x] Fixed the missing-build-directory guard by checking that the examples directory exists before counting executables; revalidated prepare and collect, including zero-output incremental no-op.
- [x] Ran final narrow code recheck; it found that the shared Docker runner macro still used an `&&` preflight chain that could skip Docker but continue to stamp publication under `.ONESHELL`.
- [x] Fixed the Docker runner macro to use separate `mkdir` and `test -w` commands under Bash `errexit`, then revalidated prepare, collect, incremental no-op, list/find, and artifact preservation.
- [x] Implement Milestone 13: final narrow recheck after the Docker runner macro fix reported no substantive findings; final validation passed and review fixes are ready to commit.
...
Milestone 12 full SCC validation succeeded. `make image-systemc-components`, `make download-systemc-components`, `make prepare-systemc-components`, and `make collect-systemc-components` completed. The prepared SCC build tree had exactly the 17 expected example executables. The collection run reported exit code 0 for all 17 examples, published 29 files, and the before/after diff for non-systemc artifacts was empty. `make list PROJECTS="systemc-components"` matched `find "$PWD/artifacts/systemc-components" -type f` exactly at 29 absolute paths, full `make list` matched `find "$PWD/artifacts" -type f` exactly at 62 absolute paths, all new files were non-empty, and the new SCC artifact total size was about 9.4 MiB.

Implementation review found that stamps in `.build/` could outlive deleted `downloads/src` or `work/src`, so `download` and `prepare` needed runtime validation before no-oping. The first fix added guarded stamp recipes, but validation exposed an old Make trap: `exit 0` in one recipe line does not stop later recipe lines unless the recipe runs in one shell. The project Makefile now uses `.ONESHELL` and Bash shell flags so guarded no-ops really skip the rest of the recipe. The same validation also found that `wc -l` output must be trimmed before comparing the expected executable count to `17`. Targeted re-review then found that `find` on a missing examples directory could abort the guard under `pipefail`; the Makefile now checks `[ -d "$examples_dir" ]` and uses `exe_count=0` when the build directory is absent so `prepare` can self-heal. A final narrow recheck found that the shared Docker runner macro still used an `&&` chain; under Bash `errexit`, failures in non-final `&&` commands can avoid exiting and let a later `touch` create a false stamp. The macro now runs `mkdir` and each `test -w` as separate commands before `docker run`, so `errexit` stops the recipe before any stamp is touched. After these fixes, `make prepare-systemc-components` and `make collect-systemc-components` both had zero-output second runs and left stamp mtimes unchanged.
...
Decision: Copy SCC outputs to final `/artifacts` only after full batch success and manifest validation, and record success with `projects/systemc-components/.build/collect.stamp`. Reason: artifact mtimes and partial per-file publication can mislead future incremental builds after an interrupted or failed run; the success stamp is removed before collection and recreated only after the full batch publishes successfully.

Decision: Use `.ONESHELL` in `projects/systemc-components/Makefile`. Reason: this project has guarded stamp recipes where `exit 0` must skip later commands in the same recipe. Without one shell per recipe, Make would run later recipe lines in separate shells and still invoke CMake or collection after a successful guard.


## Outcomes & Retrospective

Original isolated-pipeline implementation milestones are complete. Root commands run from the host and delegate to project-local pipelines. SCR1, PicoRV32, Chipyard, and `tb_complex_types` each have their own Dockerfile, Makefile, versions file, ignored download/work/build directories, and artifact list. The global devcontainer and legacy root project `.mk` files are gone. The new `systemc-components` extension has now been implemented, built, collected, reviewed, fixed, and control-reviewed with no remaining substantive findings.

Original validation produced all 33 expected artifact targets for SCR1, PicoRV32, Chipyard, and `tb_complex_types`: 14 SCR1 FST files, 3 PicoRV32 FST files, 12 Chipyard FST files, and 4 `tb_complex_types` waveform files. That historical full clean run used `make artifacts-clean && make collect`, completed successfully in 134 minutes 8 seconds, and produced 33 non-empty files totaling about 1.8 GiB under `artifacts/`. That command is destructive and was not used during the `systemc-components` extension.

SystemC-Components validation added 29 non-empty VCD/FST/FTR artifacts under `artifacts/systemc-components/` without deleting the existing 33 non-systemc artifacts. The current full artifact tree has 62 files. `make list PROJECTS="systemc-components"` matched `find "$PWD/artifacts/systemc-components" -type f` exactly at 29 absolute paths, and full `make list` matched `find "$PWD/artifacts" -type f` exactly at 62 absolute paths. A future release dry run should now stage 62 assets unless `PROJECTS` is reduced.

The original isolated-pipeline implementation review ran code, docs, and architecture lanes; findings about release stale artifacts, scoped clean behavior, image stamp metadata, README artifact accuracy, plan freshness, GitHub asset filename staging, Docker build context hygiene, and clean-release worktree checks were applied. A final targeted recheck of release staging reported no substantive findings. The `systemc-components` implementation review ran code/build, architecture/release, and docs lanes. Follow-up targeted code rechecks found and fixed stamp validation, `.ONESHELL`, missing-directory guard, and Docker preflight-chain issues. The final narrow macro recheck reported no substantive findings.
...
2026-06-05: Completed Milestone 12 by building the project image, downloading and preparing SCC, applying the patch, building all expected examples, collecting 29 non-empty VCD/FST/FTR artifacts, and verifying both project-local and full-repository listed-vs-actual artifact sets. The existing 33 non-systemc artifacts were preserved.

2026-06-05: Applied implementation-review fixes. Download and prepare stamps now validate that their source/build directories still exist and still match pinned commits before no-oping. Collection now uses a success stamp so interrupted or failed publication cannot masquerade as a completed incremental run. Documentation now records PyPI/Conan network requirements and the retrospective reflects the completed 62-file artifact tree. Follow-up validation exposed and fixed the separate-shell recipe issue by adding `.ONESHELL`; incremental collect now produces no output and leaves stamps unchanged. Targeted re-review then caught the missing-examples-directory `find|wc` failure under `pipefail`, which is fixed by treating a missing examples directory as executable count zero. A final macro recheck caught an unsafe `&&` preflight chain in `RUN_IN_CONTAINER`; it now uses separate commands so `errexit` prevents false stamps. The final narrow recheck reported no substantive findings.
